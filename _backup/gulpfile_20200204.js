...
const devServeCommand = new Command();
devServeCommand.option("dev", "-r", "--recompile", "Recompiles the Styleguide application");

const rh_theme = mandelbrot({
    skin: "blue",
    styles: ["https://use.typekit.net/vip6kss.css", "/theme/css/theme.css"],
    scripts: ["default", "https://unpkg.com/feather-icons", "/theme/js/theme.js"],
...
function theme(cb) {
    ...
    gulp.src("./assets/img/**/*").pipe(gulp.dest(fractal.web.get("static.path") + "/theme/img/"));
...
function release(cb) {
    ...
    gulp.src("./public/images/components/**/*").pipe(
        gulp.dest("./releases/" + project.version + "/images/components")
    );

    gulp.src("./public/css/components.css").pipe(
        gulp.dest("./releases/" + project.version + "/css")
    );

    gulp.src("./public/js/components.js").pipe(gulp.dest("./releases/" + project.version + "/js"));
...
function devServe(cb) {
    const devServeCmd = devServeCommand.parse(process.argv.slice(2));

    devServeCmd.recompile
        ? series(scss, images, js)() // ONLY recompiling
        : devServer(cb); // Starting the development server
...
function devServer(cb) {
    ...
    const server = fractal.web.server({
        ...
        sync: false, // BrowserSync takes care about reloading instead of Fractal
        watch: true
    });
...
// Sites building - View /sites.building.md for more detail
 ...
 * Starts dev server
 *  -> Run 'gulp dev'
 * Recompiles the Styleguide application
 *  -> Run 'gulp dev -r' or 'gulp dev --recompile'
 */
